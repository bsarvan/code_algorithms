...
//  NodesInSubTree
//
//  Created by Bharat Sarvan on 29/12/20.
...
Node * FindSubTreeNode (Node *root, int val, size_t N) {
    queue<Node *> Q;
...
int countNodesHelper(Node *root, char C, string S) {
    if (root == nullptr) {
        return 0;
    }
        
    int count = S[root->val - 1] == C? 1 : 0;
    
    for (auto c : root->children) {
        count += countNodesHelper(c, C, S);
    }
    return count;
}
...
vector<int> countOfNodes(Node* root, vector<Query> queries, string s) {
  // Write your code here
    ...
    for (auto query : queries) {
        // Find the subtree node
        Node *subtree = FindSubTreeNode(curr, query.u, s.size());
        
        // Count the nodes in subtree equal to query.c
        int count = countNodesHelper(subtree, query.c, s);
